...
def get_file_underlying(text: str):
    ...
    return match.group(1) if match else config.functions["__underlying__"]
...
def get_file_dependencies_on_plugins(text: str):
    ...
    if match:
        ...
        if match:
            ...
            return config.functions["__dependenciesOnPlugins__"]
...
def get_file_level(text: str):
    ...
    return int(match.group(1)) if match else config.functions["__level__"]
...
def sorting_plugin(plugin_dict: Dict[str, dict]) -> Dict[int, Union[list, tuple]]:
    """
    排序插件
    应该按照插件的层级和依赖关系来排序
    按照任务执行标准来排序
