...
def setPluginData(*args, data: Any) -> Union[bool, Any]:
    try:
        returnValue = getPluginData(*args[:-1])

        if isinstance(returnValue, (dict, FilemappingDict)):
            returnValue[args[-1]] = data
        else:
            setattr(returnValue, args[-1], data)
