...
def _task_run(executor: concurrent.futures.ThreadPoolExecutor, task_list: list, wrapper_list: list = None, wrapper_parameters: dict = None, parameters: dict = None):
    ...
    def func(executor, wrapper_list, wrapper_parameters, task_func, *args, **kwargs) -> concurrent.futures.Future:
        return executor.submit(decorators.wrapper_recursion(wrapper_list, task_func, wrapper_parameters), *args, **kwargs)

    wrapper_list = [decorators.parameters_wrapper] if wrapper_list is None else wrapper_list + [decorators.parameters_wrapper]
    parameters = {} if parameters is None else parameters
    wrapper_parameters = {} if wrapper_parameters is None else wrapper_parameters

    wrapper_parameters = helperFunctions.deep_update(wrapper_parameters, {"parameters": {"parameters": parameters}})
    futures = []
...
def task_recursion(task: dict, *args, **kwargs):
    if isinstance(next(iter(task)), str):
        ...
        task = helperFunctions.sort(task)
...
class EnableMultithreading(Class.EnableMultithreading):
    def __init__(self, max_workers: int = 3, callback_function=None, errorHandling=None):
        """
        多线程处理
        """
        super().__init__(max_workers=max_workers, callback_function=callback_function, errorHandling=errorHandling)
